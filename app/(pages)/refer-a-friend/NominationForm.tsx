...
/**
 * Counts up to the real total on first paint. The number is the point of the
 * section, so it animates rather than just appearing — but it starts from a
 * visible value and honours prefers-reduced-motion, so it's never blank or
 * distracting for anyone who'd rather it didn't move.
 */
function NominationCounter({
  count,
  locale,
}: {
  count: number;
  ...
  useEffect(() => {
    ...
    const from = previous.current === count ? 0 : previous.current;
    previous.current = count;
  ...
  return (
    <div className="mt-8 inline-flex items-center gap-4 rounded-3xl border border-canela/20 bg-canela-light/40 px-6 py-4">
      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-canela-dark">
        <Heart className="h-6 w-6 text-cream" fill="currentColor" />
      </div>
      <div>
        <p className="font-display text-3xl leading-none text-ink tabular-nums md:text-4xl">
          {shown.toLocaleString(locale === "es" ? "es-MX" : "en-CA")}
        </p>
        <p className="mt-1 text-sm text-ink-soft">
          {locale === "es"
            ? shown === 1
              ? "historia compartida por la comunidad"
              : "historias compartidas por la comunidad"
            : shown === 1
            ? "story shared by the community"
...
const t = {
  ...
  formIntro:
    locale === "es"
      ...
      : "Tell us who you want to nominate and why. We will review each story with respect and care.",
    ...
    return (
      <div className="container-bakery py-20 md:py-28">
        <div className="mx-auto max-w-xl text-center">
          ...
          {/* The moment their own nomination lands is when the number means
              the most, so it's shown here too, already counting theirs. */}
          <NominationCounter count={count} locale={locale} />
